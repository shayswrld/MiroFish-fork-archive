  ...
  <div class="process-page">
    ...
    <nav class="navbar">
      ...
      <div class="nav-center">
        ...
        <div class="step-name">{{ t('process.buildingPhase') }}</div>
      </div>
    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph visualization (shown as long as data exists) -->
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ t('process.buildingPhase') }}</div>
...
const statusText = computed(() => {
  if (error.value) return t('process.graphBuildFailed')
...
const goToNextStep = () => {
  // TODO: Enter environment setup step
    alert(t('process.envSetupInProgress'))
}
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files to upload, please go back to home and try again'
    loading.value = false
    return
  }
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || t('process.startFailed')
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || t('process.graphBuildFailed')
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = t('process.graphBuildFailedWithError', { error: err.message || t('process.unknownError') })
    buildProgress.value = null
  }
}
